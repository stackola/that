...
export function uploadImage(path, callback = null, fileType = "jpg") {
	...
	imageRef.putFile(path).then(function(snapshot) {
		...
		registerImage({ path: snapshot.ref, url: snapshot.downloadURL }).then(() => {
			if (callback) {
				callback({ path: snapshot.ref, url: snapshot.downloadURL, user:getUID() });
			}
		});
	});
}
