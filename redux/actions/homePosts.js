	...
	return (dispatch, getState) => {
		return getHomePosts().then(list=>{
            return Promise.all(list.map((g)=>{
                return firebase.firestore().collection("groups").doc(g.slug).collection("posts").orderBy("time", "DESC").limit(25).get().then((s)=>{return s._docs})
            })).then((s)=>{console.log("got them snaps does", dispatch(setHomePosts(s.flat()))); return;});
        });
    }
}